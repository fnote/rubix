...
export class StorageService {
	...
	private addToPersistTimeArr(key: string , persistTime: number): void {
		...
		if (this.persistIteration % ITERATIONS_PER_LOCALSTORAGE_SIZE_CHECK === 0
		&& JSON.stringify(localStorage).length > MAX_STORAGE_LIMIT) {
			persistTimeArr = persistTimeArr.sort(
				function(a: {key: string , persistTime: number } ,
					b: {key: string , persistTime: number }): number {
		...
		// if (this.persistIteration % 100 === 0) {
		// TODO: [Malindu] Uncomment above if performance is low & do this on browser close also
