...
// VML fallback
    ...
    H.SVGRenderer.prototype.symbols.reset = function(x,y,w,h){
        var p = 1;
        return [
            'M', x, y + h/2,
            'A', w/2, w/2, 0, 0, 0, x + w/2, y + h,
            'M', x + w/2, y + h,
            'A', w/2, w/2, 0, 0, 0, x+ w, y + h/2,
            'M', x+ w, y + h/2,
            'A', w/2, w/2, 0, 0, 0, x + w/2, y,
            'M', x + w/2, y,
            'A', w/2, w/2, 0, 0, 0, x + w* 11/32, y + h* 4/32,
            'M', x + w* 13/32, y + h* 1/32,
            'L', x + w* 21/32, y + h* 12/32,
            'M', x + w* 11/32, y + h* 1/32,
            'L', x + w* 25/32, y - h* 7/32
        ];
    };
// VML fallback
    if(H.VMLRenderer) {
        H.VMLRenderer.prototype.symbols.reset = H.SVGRenderer.prototype.symbols.reset;
    }
